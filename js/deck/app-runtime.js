window.DeckAppRuntime = {
  ...
  const forcedIndex = Number.isFinite(forcedSlide)
    ...
    : null;
  const store = window.DeckModules.stateStore.createDeckStore({
    slides,
    storage: window.localStorage,
    sessionStorage: window.sessionStorage,
    forcedIndex,
    storageKey: STORAGE_KEY,
    pathwayStorageKey: PATHWAY_STORAGE_KEY,
    sessionConnectedKey: SESSION_KEY_CONNECTED,
    sharedResponseSlideId: SHARED_RESPONSE_SLIDE_ID
  });

  const persisted = store.loadPersistedState();
  const persistedPathway = store.loadPathwaySelection();
  const persistState = (nextState) => store.persistState(nextState);
  const state = {
    ...
    apiConnected: store.getSessionConnected(),
    testingConnection: false
  };
  const exportService = window.DeckModules.exportService.createExportService({
    state,
    slides,
    deckTitleProvider: () => window.DECK_DATA.title
  });
  ...
  const inclusionResolver = window.DeckModules.navigationRouter.createInclusionResolver({
    slides,
    state,
    alwaysIncludedSectionPrefixes: ALWAYS_INCLUDED_SECTION_PREFIXES
  });
  const isAlwaysIncludedSection = inclusionResolver.isAlwaysIncludedSection;
  const isSlideIncludedByPathway = inclusionResolver.isSlideIncludedByPathway;
  const findNextIncludedIndex = inclusionResolver.findNextIncludedIndex;

  function persistPathwaySelection(pathwayId, selectedSections) {
    store.persistPathwaySelection(state, pathwayId, selectedSections);
    sectionSummaries = buildSectionSummaries();
  }

  function saveResponse(slideId, fieldKey, value) {
    store.saveResponse(state, slideId, fieldKey, value);
  }

  function saveSharedResponse(fieldKey, value) {
    store.saveSharedResponse(state, fieldKey, value);
  }

  function getSharedResponse(fieldKey, fallback = null) {
    return store.getSharedResponse(state, fieldKey, fallback);
  }

  function getResponse(slideId, fieldKey, fallback = null) {
    return store.getResponse(state, slideId, fieldKey, fallback);
  }
  ...
  const aiService = window.DeckModules.aiService.createAiService({});
  ...
  function createInteractionContext() {
    return window.DeckModules.renderInteractions.createInteractionContext({
      saveResponse,
      getResponse,
      setFeedback,
      hideFeedback,
  ...
  function resolveInteractionType(slide) {
    return window.DeckModules.renderInteractions.resolveInteractionType(slide);
  }
  ...
  function renderTextSlide(card, slide) {
    ...
    if (slide.id === "06.1") {
      ...
      exportBtn.addEventListener("click", () => {
        exportService.downloadDeckResponsesPdf({ filename: "deck-reflections-export.pdf" });
  ...
  function renderGrounding321(card, slide) {
    ...
    sendBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        const mappedError = aiService.mapDeckAiError(error, "Unable to fetch AI response right now.");
  ...
  const interactionRenderer = window.DeckModules.renderInteractions.createInteractionRenderer({
    getInteractionModule,
    builtInRenderers,
    createContext: createInteractionContext,
    moduleTypes: MODULE_INTERACTION_TYPES,
    renderUnknownInteraction
  });
  const renderSlideByInteractionType = interactionRenderer.renderSlideByInteractionType;
  ...
  const router = window.DeckModules.navigationRouter.createDeckRouter({
    slides,
    pairedGroups,
    state,
    getResponse,
  ...
  const getNextIndex = router.resolveNextIndex;
  ...
  const coreRenderer = window.DeckModules.renderCore.createRenderCore({
    state,
    slides,
    elements: el,
    sectionSummariesRef: () => sectionSummaries,
  ...
  const render = () => coreRenderer.render();
  ...
  window.deckHooks = {
    ...
    getSlides: () => slides,
    exportResponses: () => exportService.exportResponses(),
    onBeforeExport: (fn) => {
